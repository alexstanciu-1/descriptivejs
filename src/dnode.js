...
export class DNode
{
	...
	init ($dom, $parent, $attrs, $run)
	{
		...
		// console.log('init_data', this.dom, this);
	...
	expChanged($force, $args, $exp_return)
	{
		...
		// console.log('expChanged :: ' + $what + ' :: ' + this.id + ' :: ' + JSON.stringify(this.attrs));
		
		if ($what[0] === '@')
		{
			// console.log('expChanged', $what, this);
			if (!$has_exp)
			{
		...
		else if ($what === 'model')
		{
			...
			if ((this.dom.tagName === 'INPUT') || (this.dom.tagName === 'TEXTAREA'))
			{
				...
				{
					// console.log('model change ... fssss', this.dom.checked, this.dom.value, $exp_return, this.exps['model'] + '');
			...
			// console.log('model', $exp_return, this.dom, this);
		}
		else if ($what === 'show')
		{
			if (!$has_exp)
	...
	init_data($force = false)
	{
		...
		if ((!this._obj) || $force)
		{
			if (this._new_ctx || ($q_data_str && (this._new_ctx = true)))
			{
				this._obj = {};
				if (this.parent)
	...
	setupExp($expression, $type, $with_return, $run_expr, $run_expr_watch, $trigger_changes)
	{
		...
		if (!$func)
		{
			// console.log(` with ($data) { ${$with_return ? 'return' : ''} ${$expression}; } `);
		...
		*/
		...
		try
		{
			if (($with_watch === undefined) || $with_watch)
			{
				...
				/*if (($type === 'if'))
				{
					console.log('attaching watch: ' + $type + ' on ' + this.id + JSON.stringify(this.attrs) + " | on: ", this._obj);
					// alert('hmmmm ...');
				}*/
			}
			// safer-eval
			...
			if (($with_watch === undefined) || $with_watch)
			{
				if (($type === 'if'))
				{
					console.log('after watch: ' + $type + ' on ' + this.id + JSON.stringify(this.attrs) + " | on: ", this._obj);
					alert('halt!');
				}
			}
			*/
		...
		{
			console.log('executeExp@exception[notice]: ', $ex);
			$ret = $ex;
	...
	ctrl_dynamic($val, $load_js)
	{
		...
		else if ($ext[1] === 'tpl')
		{
			(async () => {
				...
				// console.log($resp, $local_element);
				document.body.insertAdjacentHTML("beforeend", $html);
				
				// we need to do some proper wrapping here `lastElementChild` not a great idea !!!
				
				var $dom = document.body.lastElementChild;
				
				var $tpl_ctx = new DNode();
				
				// @TODO - make sure we boot it corectly
				// ways to link it to data !
				$tpl_ctx.init($dom, this.parent, $tpl_ctx.dom_get_attrs($dom), true);
				
				if ($load_js)
				{
					const $local_val = $val;
					// const $local_element = this.dom;
					await import($load_js);
				}
				
				// $tpl_ctx.data.test_binds = 'alex likes this #1';
				
				/*
				$tpl_ctx.getData().test_binds = 'alex likes this';
				console.log('$tpl_ctx', $tpl_ctx);
				*/
